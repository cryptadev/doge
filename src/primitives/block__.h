...
class CAuxPow {
  ...
  public:
    ...
    inline CAuxPow() : tx(std::move(MakeTransactionRef())) { }
    ...
    template<typename Stream, typename Operation>
        inline void SerializationOp (Stream& s, Operation ser_action) {
        READWRITE(tx);
        uint256 hashBlock; if (!ser_action.ForRead()) hashBlock = parentBlock.GetHash(); READWRITE(hashBlock);
        READWRITE(vMerkleBranch);
        int nIndex; if (!ser_action.ForRead()) nIndex = 0; READWRITE(nIndex);
