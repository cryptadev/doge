...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    int32_t AuxpowChainId = chainparams.GetConsensus().nAuxpowChainId;
    pblock->SetBaseVersion(VERSIONBITS_LAST_OLD_BLOCK_VERSION, AuxpowChainId);
    // -regtest only: allow overriding block.nVersion with
    // -blockversion=N to test forking scenarios
    if (chainparams.MineBlocksOnDemand())
        pblock->SetBaseVersion(gArgs.GetArg("-blockversion", pblock->GetBaseVersion()), AuxpowChainId);
...
int generateCoin (int nThreads) {
    if (nThreads <= 0) { generatePoWCoin (0); }
