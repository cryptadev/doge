                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s DOGE additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight < consensusParams.SimplifiedRewardsHeight) {
        const CBlockIndex* pi = chainActive[nHeight-1];
        assert (pi);
        uint256 prevHash = pi->GetBlockHash();
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if ((chainparams.GetConsensus().CSVHeight > 0) &&
            (pindex->pprev->nHeight >= chainparams.GetConsensus().CSVHeight)) {
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (!fPruneMode && !fReindex && (chainActive.Height() > 50000)) {
            const CBlockIndex* pi = chainActive.Tip();
            if (pi) pi = pi->GetAncestor(chainActive.Height() - 50000);
            int newfn = 0;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if ((nHeight > consensusParams.DisallowLegacyBlocksHeight) && block.IsLegacy())
    ...
    if ((nHeight < consensusParams.DisallowLegacyBlocksHeight) && block.IsAuxpow())
        return state.DoS(100, error("%s : auxpow blocks are not allowed at height %d, parameters effective from %d",
                                    __func__, pindexPrev->nHeight + 1, consensusParams.DisallowLegacyBlocksHeight),
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if ((consensusParams.CSVHeight > 0) && (nHeight >= consensusParams.CSVHeight)) {
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if  (pindex->nHeight > chainparams.GetConsensus().DisallowLegacyBlocksHeight) {
        int auxsize = 0;
        if (block.auxpow) auxsize = GetSerializeSize(*block.auxpow, SER_NETWORK, PROTOCOL_VERSION);
        if (auxsize == 0) LogPrintf("       Aux of %d is NULL\n", pindex->nHeight);
        if (auxsize > 1000) 
            LogPrintf("       Aux of %d is: size=%d, vin.size=%u, vout.size=%u, vMerkle = %d, vChainMerkle = %d, nChain = %d\n",
                pindex->nHeight, auxsize, block.auxpow->tx->vin.size(), block.auxpow->tx->vout.size(),
                block.auxpow->vMerkleBranch.size(), block.auxpow->vChainMerkleBranch.size(), block.auxpow->nChainIndex);
    }
